...
Docker Compose 環境向けの `.localtest.me` サブドメインルーター。
...
|------|------|-----------------|
| ポート番号の記憶 | `localhost:3000` は何？ | `frontend.myapp.localtest.me` で明確 |
...
```
Browser: http://frontend.myapp.localtest.me:1355
                    │
         .localtest.me → 127.0.0.1 (パブリック DNS サービス)
                    │
        portless-docker proxy (:1355)
         frontend.myapp.localtest.me → :41023
         api.myapp.localtest.me      → :41056
                │                │
...
4. ポート 1355 のリバースプロキシで `<service>.<project>.localtest.me` をルーティング
...
それ以外のポート（80, 3000, 8080 など）は HTTP サービスとして `<service>.<project>.localtest.me:1355` 経由でアクセスできます。`<project>` はディレクトリ名から自動的に決定されます（アンダースコアはハイフンに、大文字は小文字に変換）。
...
#   http://frontend.myapp.localtest.me:1355
#   http://api.myapp.localtest.me:1355
```
...
| プレースホルダー | 値 |
|---------------|---|
| `{{service.host}}` | `service.project.localtest.me` |
| `{{service.port}}` | プロキシのリッスンポート（HTTP）/ 割り当てポート（TCP） |
| `{{service.url}}` | `http://service.project.localtest.me:1355`（HTTP）/ `localhost:4xxxx`（TCP） |
...
`.localtest.me` はパブリック DNS サービスで、すべてのサブドメインが `127.0.0.1` に解決されます。Safari/WebKit を含むすべてのブラウザで動作し、`/etc/hosts` の変更や DNS 設定は不要です。
