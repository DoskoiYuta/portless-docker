...
portless-docker は、Docker Compose 環境に特化した `.localhost` サブドメインルーターである。
...
|------|------|----------------------|
| ポート番号の記憶 | `localhost:3000` は何のサービス？ | `frontend.localhost` で明確 |
...
$ portless-docker up
  ...
  ④ グローバルプロキシにルート登録
     → frontend.localhost:1355 → 127.0.0.1:41023
     → api.localhost:1355      → 127.0.0.1:41056
...
│                                                  │
│  frontend.localhost → 127.0.0.1:41023            │
│  api.localhost      → 127.0.0.1:41056            │
└───────┬──────────────────┬──────────────────────┘
        │                  │
        ▼                  ▼
  ┌──────────┐       ┌──────────┐
...
Compose: /home/user/myproject/docker-compose.yml
  ...
  http://frontend.localhost:1355  →  :41023 (container :3000)
  http://api.localhost:1355       →  :41056 (container :8080)
...
Active routes:
 ...
 /home/user/project-b
   http://web.localhost:1355       →  :41102 (container :3000)
   http://worker.localhost:1355    →  :41103 (container :9000)
```
...
| サービス名 | URL |
|-----------|-----|
| `frontend` | `http://frontend.localhost:1355` |
| `api` | `http://api.localhost:1355` |
| `my-service` | `http://my-service.localhost:1355` |
| `web_app` | `http://web-app.localhost:1355` |
...
| ホスト名競合 | `"frontend.localhost" is already registered by /other/project` |
